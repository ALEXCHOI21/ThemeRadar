...
2. 찌라시와 노이즈를 100% 배제하고, 제공된 '당일 실시간 투자주체별 순매수 동향'의 구체적 수치(예: +22,535억 -> +2.2조 원 매수, -2,080억 -> -2,080억 매도 등 가독성 좋은 단위 변환 적극 사용)를 바탕으로 다음 항목을 작성하십시오:
   - 2번 항목: 코스피(KOSPI) 수급 동향 (개인, 외국인, 기관의 순매수/순매도 구체적 수치 포함)
   - 3번 항목: 코스닥(KOSDAQ) 수급 동향 (개인, 외국인, 기관의 순매수/순매도 구체적 수치 포함)
3. 이미지 카드뉴스(1080x1080)에 직접 그려질 텍스트이므로 가독성을 위해 각 문장은 35자 이내로 대단히 명확하고 간결해야 합니다.
...
{{
  ...
  "bullets": [
    "1. 코스피 요약 (지수 및 등락률)",
    "2. 코스피 수급 (개인/외인/기관 수치 필수)",
    "3. 코스닥 수급 (개인/외인/기관 수치 필수)",
    "4. 매크로 환율 요약",
    "5. 대응 권고 및 전략 제안"
  ]
}}
